...
| **Retention t_dispatch_history** | Реализовано (неделя 3): `alerting.maint_dispatch_partitions` при старте движка и раз в сутки нарезает недельные партиции и дропает старше `ALERTING_DISPATCH_RETENTION_DAYS` (по умолчанию 90; лимиту нужно 30 дней, аудиту хватает 90). Дальнейшее улучшение — заменить самописную функцию на `pg_partman` при росте числа партиций. |
