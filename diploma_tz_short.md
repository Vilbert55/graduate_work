# Дипломный проект — автоматизированный сервис уведомлений на основе событий
...
| `activity-tracker-service` | Новый event_type `recommendation` (Kafka-топик `recommendations`): авторизованный `POST /ugc/api/v1/events/recommendation` и публичный `GET /ugc/email/click` — ссылка из письма, клик публикует событие `action=clicked` без JWT. | Замыкает контур «правило -> письмо -> клик по ссылке -> факт обратно в StarRocks». В Superset видна доля перешедших по ссылке (`mv_rule_conversion`). |
...
Итого: событие в Kafka -> агрегат в Materialized view -> выборка SQL-правила -> задача в `notifications-service` -> письмо со ссылкой -> клик по ссылке -> событие обратно в StarRocks -> воронка отклика в Superset.
...
| Синхронизация dim-таблиц | StarRocks JDBC Catalog + `SUBMIT TASK` | Нативный механизм StarRocks, аналог Routine Load для пакетной загрузки. Отдельный Python ETL-сервис избыточен для нескольких таблиц с полной перезаливкой раз в час. |
