...
- **Неделя 1.** Тема, ТЗ, архитектура, созвон с наставником. (Сейчас.)
- **Неделя 2.** Миграция `auth-service`, утилиты `demo-seeder` / `event-trigger`, init-контейнер StarRocks (DDL + JDBC Catalog + `SUBMIT TASK`), миграции схемы `alerting`, SQL-функции управления правилами, минимальный движок (заготовка), Superset. Аналитическая записка.
- **Неделя 3.** Полный движок: цикл планировщика, лимит уведомлений, `t_dispatch_history` с партиционированием, тестовый прогон, ручной запуск, представления, восстановление после сбоя, сквозной тест с Mailpit. Демо ревьюеру.
- **Неделя 4.** Метрики, пограничные случаи (StarRocks или notifications недоступны, некорректный SQL), документация, отладка скриптов демонстрации, презентация, защита.
