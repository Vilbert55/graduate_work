...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # is_dry_run: тестовый прогон (adm_dry_run_rule). Нужен recovery, чтобы
        # не «дослать» прерванный dry-run реальными письмами.
    ...
    # Первичная нарезка партиций t_dispatch_history (эта + следующая неделя).
    # Функция загружена шагом _load_sql_dir("functions").
    op.execute("SELECT alerting.maint_dispatch_partitions(90)")
